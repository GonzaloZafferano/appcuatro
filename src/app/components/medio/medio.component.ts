...
export class MedioComponent  implements OnInit {
  ...
  rutasImagenes: string[] = [
    '/assets/imagenes/destornillador.png',
  ...
  ngOnInit() {
    ...
    this.firestore.obtenerGanadoresMedio().subscribe((info) => {
      this.ganadores = info;
    });
  }
  ...
  verificarPuestos(segundos: number, milisegundos: number) {
    ...
    if (this.ganadores.length < 5) {
      ...
      for (let i = 0; i < this.ganadores.length; i++) {
        if (segundos < this.ganadores[i].segundos ||
          ...
          this.firestore.eliminarGanadorMedio(ultimoPuesto.id);
          break;
        }
      }
    }
    if (hayGanador) {
      ...
      this.firestore.guardarMedio(ganador);
    }
    return hayGanador;
  }
